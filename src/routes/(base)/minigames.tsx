  ...
  return (
    <>
      ...
      <div class="grid md:grid-cols-2 gap-y-6 py-20">
        ...
        >
          <div class="h-56 w-full flex bg-base-200 group-hover:bg-on-base/5 rounded-2xl mb-6 rotate-3 group-hover:rotate-0 transition-all relative overflow-hidden">
            <div class="grid grid-cols-12 gap-2 p-2 w-max absolute-center opacity-30 group-hover:opacity-70 transition-opacity">
              {palette.map((row) => (
                <div class="grid grid-rows-5 gap-2">
                  {row.map((color) => (
                    <div class={`h-10 w-10 sm:h-12 sm:w-12 md:h-10 md:w-10 rounded-md ${color}`} />
                  ))}
                </div>
              ))}
            </div>
